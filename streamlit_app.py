...
def main():
    ...
    with tab3:
        ...
        # 准备数据
        if selected_trade_id == '全部':
            picked_data = otc_trade[['观察日','TRADE_KEYWORD.期权特殊类型','标的名称','期权估值（报送）','期权估值 NPV','期权费估值','DELTA(期权)','GAMMA','VEGA（1%）','THETA','RHO','总盈亏']].groupby(['观察日']).sum()
            cols_to_normalize = ['期权估值（报送）','期权估值 NPV','期权费估值','DELTA(期权)','GAMMA','VEGA（1%）','THETA','RHO','总盈亏']
        else:
